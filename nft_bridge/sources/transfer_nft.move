module nft_bridge::transfer_nft {
    ...
    public entry fun transfer_nft_entry(
        ...
        if (wormhole_fee > 0) {
            ...
            wormhole_fee_coins = coin::zero<CedraCoin>();
        };
        transfer_nft(
            token,
            wormhole_fee_coins,
    ...
    fun lock_or_burn(token: Token): (
        ...
        // NOTE: the way cedra tokens are designed, it is possible to mint
        ...
        // The symbol field will be set to empty for cedra native NFTs (as they
        ...
        if (state::is_wrapped_asset(&token_id)) {
            ...
            // given time, so they effectively behave as NFTs outside of Cedra,
            // even when they're tecnhically fungible on cedra.
