...
specification](../../whitepapers/0006_nft_bridge.md) on Cedra, written in the
...
a good overview of how Tokens are specified on Cedra, but we review the relevant
parts here.
...
information on Cedra, since the creator address itself is already 32 bytes, and
...
When transferring an NFT from a collection on a foreign chain to Cedra, a
...
transferring ones that are non-0. However, many real-world NFT projects simply
...
case is not observable outside of Cedra.
...
Contract upgrades also require governance VAAs. In the case of Cedra, the VAA
