module token_bridge::transfer_tokens {
    ...
    public fun transfer_tokens<CoinType>(
        coins: Coin<CoinType>,
        wormhole_fee_coins: Coin<CedraCoin>,
        recipient_chain: U16,
    ...
    fun transfer_tokens_internal<CoinType>(
        ...
        if (!coin::is_account_registered<CedraCoin>(@token_bridge)) {
            coin::register<CedraCoin>(&state::token_bridge_signer());
        };
...
module token_bridge::transfer_tokens_test {
    use cedra_framework::coin::{Self, Coin};
    use cedra_framework::string::{utf8};
    ...
    fun setup(
        ...
        coin::register<CedraCoin>(token_bridge); //how important is this registration step and where to check it?
        coin::destroy_burn_cap(burn_cap);
        coin::destroy_mint_cap(mint_cap);
    }

    // test transfer wrapped coin
    #[test(cedra_framework = @cedra_framework, token_bridge=@token_bridge, deployer=@deployer)]
    fun test_transfer_wrapped_token(cedra_framework: &signer, token_bridge: &signer, deployer: &signer) {
        setup(cedra_framework, token_bridge, deployer);
    ...
    #[test(cedra_framework = @cedra_framework, token_bridge=@token_bridge, deployer=@deployer)]
    fun test_transfer_native_token(cedra_framework: &signer, token_bridge: &signer, deployer: &signer) {
        setup(cedra_framework, token_bridge, deployer);
    ...
    #[test(cedra_framework = @cedra_framework, token_bridge=@token_bridge, deployer=@deployer, user=@0xBEEF)]
    fun test_transfer_with_payload(cedra_framework: &signer, token_bridge: &signer, deployer: &signer, user: &signer) {
        setup(cedra_framework, token_bridge, deployer);

        let my_coins = init_my_token(token_bridge, 10000);
        cedra_account::deposit_coins(std::signer::address_of(user), my_coins);
